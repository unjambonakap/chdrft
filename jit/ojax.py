...
class JaxDumpFileSpec(cmisc.PatchedModel):
  ...
  def ir_with_funcname(self, funcname: str) -> str:
    res, cnt = re.subn(r'(define void @)(.*)\(', f'\\1{funcname}(', self.content)
    assert cnt == 1
    return res
...
class JaxIRExtractor(cmisc.PatchedModel):
  ...
  @classmethod
  def Make(cls, files) -> "JaxIRExtractor":
    ...
    return cls(name2modules=name2modules)
...
class JaxBufAsst(cmisc.PatchedModel):
  ...
  def Make(cls, spec: JaxDumpFileSpec) -> "JaxBuxAsst":
    part = re.search(
        r'BufferAssignment:(?P<data>.*?)Total bytes used', spec.content, re.MULTILINE | re.S
    ).groupdict()['data']
    lines = part.strip().splitlines()
    params = []
    for l in lines:
      mx = re.search(r'allocation (?P<num>\d+): (?P<rem>.*):', l)
      if not mx: continue
      g1 = mx.groupdict()
      num = g1['num']
      props = A(num=[num])
...
class LLVMHelper(cmisc.PatchedModel):
  bin_dir: str = '/usr/bin'
  ctx: ExitStack = cmisc.pyd_f(lambda: app.global_context)
...
class JaxCodeGen(cmisc.PatchedModel):
  ...
  def generate_so(self):
    obj = self.generate()
    return self.llvm_helper.obj2so(obj)

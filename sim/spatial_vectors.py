...
class SpatialVector(cmisc.PatchedModel):
  ...
  def to_spatial_momentum(self, i: Inertial) -> SpatialVector:
    return SpatialVector(dual=1, f=self.v * i.m, m=i.data @ self.m)
...
class Transform_SV(cmisc.PatchedModel):
  ...
  def change_space(self, wl: Transform) -> Transform:
    winv = wl.inv
  ...
  @classmethod
  def FromTransform(cls, wl: Transform, dual: bool) -> Transform:
  ...
  @classmethod
  def MakeSpatialTensor(cls, i: Inertial) -> Transform:
    res = np.zeros((6, 6))
    res[:3, :3] = np.identity(3) * i.m
    res[3:, 3:] = i.data
    return cls(data=res, kind=TransformKind.FF)

  def derivate(self, v: SpatialVector) -> Transform:
  ...
  def d_tsf_ab(x_ab: Transform, a: SpatialVector, b: SpatialVector) -> Transform:
